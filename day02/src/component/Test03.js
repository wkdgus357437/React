...
const Test03 = () => {
  const [name, setName] = useState("홍길동"); /*state 변수잡기 */
  ...
  return (
    <div>
      ...
      <p>
        <button onClick={()=>onName()}>코난으로 이름변경</button>
        <button onClick={()=>onAge(35)}> 35살로 변경</button>
        <button onClick={()=>onColor()}>yellow 변경</button>
